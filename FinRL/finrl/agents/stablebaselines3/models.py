...
class DecayingActionNoise(NormalActionNoise):
    """
    Action noise that decays its standard deviation over time.
    """
    def __init__(self, mean, sigma, sigma_end=0.01, decay_steps=500_000):
        super().__init__(mean, sigma)
        self._sigma_start = np.array(sigma)
        self._sigma_end = np.array(sigma_end * np.ones_like(mean))
        self._decay_steps = decay_steps
        self._step = 0

    def __call__(self) -> np.ndarray:
        progress = min(1.0, self._step / self._decay_steps)
        self._sigma = self._sigma_start - progress * (self._sigma_start - self._sigma_end)
        self._step += 1
...
class DRLAgent:
    ...
    @staticmethod
    def DRL_prediction(model, environment, deterministic=True, vec_env_path=None):
        ...
        if vec_env_path is not None and os.path.exists(vec_env_path):
            test_env = VecNormalize.load(vec_env_path, test_env)
            test_env.training = False     # do not update stats at test time
            test_env.norm_reward = False  # we only normalize observations during test
